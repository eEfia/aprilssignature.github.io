...
self.addEventListener("install",event=>event.waitUntil(caches.open(CACHE).then(c=>Promise.all(CORE.map(async utry{const r=await fetch(u,{cache:"reload"});if(r.ok)await c.put(u,r.clone())}catch(_){}}))).then(()=>self.skipWaiting())));
