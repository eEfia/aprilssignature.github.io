...
const DEFAULT_PRODUCTS = [
    ...
    ["Streetwear","T-shirts and shorts",16,"Sets"],["Streetwear","T-shirts and sweatpants",17,"Sets"],["Streetwear","Sweatshirts and shorts",18,"Sets"],["Streetwear","Sweatshirt and pants",19,"Sets"],
...
async function ensureStreetwearCatalogue() {
    try {
        ...
        const legacyRenames = new Map([
            ...
            ["t shirts and shorts", "T-shirts and shorts"], ["t shirt and shorts", "T-shirts and shorts"], ["t shirt sweatpants set", "T-shirts and sweatpants"], ["t shirt and sweatpants", "T-shirts and sweatpants"],
        ...
        const requiredCatalogueEntries = [
        ...
        for (const [category,name,order,subcategory] of requiredCatalogueEntries) {
            const key=productKeyFromName(name);
            const exists=await db.from("settings").select("id").eq("setting_key",key).limit(1);
            if (!exists.error && !exists.data?.length) await db.from("settings").insert({setting_key:key,setting_value:JSON.stringify({name,category,public_price:null,subcategory,notes:"",display_order:order,active:true,catalogue_key:catalogueKeyFromName(name)}),updated_at:new Date().toISOString()});
        }

        const embellishmentRenames = new Map([
            ["3d patches", "3D / Rhinestone Patches"],
            ["3d/rhinestone patches", "3D / Rhinestone Patches"],
            ["hand cut", "Hand / Laser Cuts"],
            ["hand/laser cuts", "Hand / Laser Cuts"]
        ]);
        for (const row of rows) {
            let item={}; try{item=JSON.parse(row.setting_value||"{}")}catch(_){continue;}
            if (String(item.category||"").trim().toLowerCase() !== "embellishment services") continue;
            const rename=embellishmentRenames.get(String(item.name||"").trim().toLowerCase());
            if (rename && rename !== item.name) {
                const newKey=productKeyFromName(rename);
                const existing=await db.from("settings").select("id").eq("setting_key",newKey).limit(1);
                if (existing.data?.length && String(existing.data[0].id)!==String(row.id)) await db.from("settings").delete().eq("id",row.id);
                else await db.from("settings").update({setting_key:newKey,setting_value:JSON.stringify({...item,name:rename,catalogue_key:catalogueKeyFromName(rename)}),updated_at:new Date().toISOString()}).eq("id",row.id);
            }
        }
...
function setupDirectCustomerLinks() {
    const links = {
        ...
        review: "https://g.page/r/CcD7hxB7NK7pEAE/review"
    };
    ...
    const labels = {
        ...
        shop: "Shop",
        review: "Send us a Google review"
    };
...
function buildInvoiceLinesFromQuote(row, details, priceMap) {
    ...
    if (Array.isArray(details?.selectedServices) && details.selectedServices.includes("Address")) {
        const a=details.address||{};
        if (a.request || a.sizeMeasurements || a.colour || Number(a.quantity||0)>0) {
            lines.push({description:"Address", quantity:Math.max(1,Number(a.quantity||1)), unitPrice:invoicePriceFor(priceMap,"Address"), details:[a.sizeMeasurements,a.colour,a.request].filter(Boolean).join(" • ")});
        }
    }
...
async function showPdfShareOptions(kind, paper, options, text, phone, email) {
    const existing = document.getElementById("aprilsPdfShareOptions");
    existing?.remove();
    const modal = document.createElement("div");
    modal.id = "aprilsPdfShareOptions";
    modal.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,.58);z-index:2147483646;display:flex;align-items:center;justify-content:center;padding:20px";
    const title = kind === "receipt" ? "Share Receipt PDF" : "Share Invoice PDF";
    modal.innerHTML = `<div style="background:#fff;max-width:430px;width:100%;padding:24px;border-radius:10px;box-shadow:0 10px 40px rgba(0,0,0,.25)">
        <h2 style="margin-bottom:8px;color:#008c95">${escapeHTML(title)}</h2>
        <p style="margin-bottom:18px">Choose where you want to send or save the exact PDF currently shown.</p>
        <div style="display:grid;gap:10px">
            <button type="button" class="primary" data-pdf-share="whatsapp">WhatsApp</button>
            <button type="button" class="secondary" data-pdf-share="email">Email</button>
            <button type="button" class="secondary" data-pdf-share="download">Download PDF</button>
            <button type="button" class="danger" data-pdf-share="cancel">Cancel</button>
        </div>
    </div>`;
    document.body.appendChild(modal);
    const close=()=>modal.remove();
    modal.addEventListener("click",e=>{if(e.target===modal)close();});
    modal.querySelector('[data-pdf-share="cancel"]').onclick=close;
    modal.querySelectorAll('[data-pdf-share]:not([data-pdf-share="cancel"])').forEach(btn=>btn.onclick=async()=>{
        const action=btn.dataset.pdfShare;
        btn.disabled=true;
        try {
            const blob=await pdfFromVisibleElement(paper,options);
            if (action === "download") {
                await downloadPdfBlob(blob,options.filename);
                message("PDF downloaded successfully.","success");
            } else if (action === "whatsapp") {
                await downloadPdfBlob(blob,options.filename);
                window.open(customerWhatsAppUrl(phone,text),"_blank","noopener,noreferrer");
                message("The PDF was downloaded and the WhatsApp chat was opened. Attach the downloaded PDF there.","success");
            } else if (action === "email") {
                await downloadPdfBlob(blob,options.filename);
                const address=String(email||"").trim();
                const subject=encodeURIComponent(kind === "receipt" ? "Aprils Signature Payment Receipt" : "Aprils Signature Invoice");
                window.location.href=`mailto:${encodeURIComponent(address)}?subject=${subject}&body=${encodeURIComponent(text)}`;
                message("The PDF was downloaded and the email composer was opened. Attach the downloaded PDF there.","success");
            }
            close();
        } catch(error) {
            console.error(error);
            message("The PDF could not be prepared for sharing. Use Download PDF instead.","error");
            btn.disabled=false;
        }
    });
}
...
async function generateInvoicePdf(share) {
    ...
    try {
        ...
        if (share) {
            ...
            await showPdfShareOptions("invoice", paper, options, getGeneratedInvoiceShareText(), document.getElementById("generatedInvoicePhone")?.value || "", document.getElementById("generatedInvoiceEmail")?.value || "");
            return false;
        }
        const blob = await pdfFromVisibleElement(paper, options);
        await downloadPdfBlob(blob, options.filename);
...
async function generateReceiptPdf(share) {
    ...
    try {
        ...
        if (share) {
            ...
            await showPdfShareOptions("receipt", paper, options, getGeneratedReceiptShareText(), document.getElementById("generatedReceiptPhone")?.value || "", document.getElementById("generatedReceiptEmail")?.value || "");
            return false;
        }
        const blob = await pdfFromVisibleElement(paper, options);
        await downloadPdfBlob(blob, options.filename);
...
function buildQuoteDetailRows(row, details) {
    ...
    if (selected.includes("Streetwear") && details.streetwear && typeof details.streetwear === "object") {
        Object.entries(details.streetwear).forEach(([key, item]) => {
            ...
            const detailText = typeof item === "object"
                ? [(item.size || item.measurements) ? `${item.size ? "Size" : "Measurements"}: ${item.size || item.measurements}` : "", item.colour ? `Colour: ${item.colour}` : ""].filter(Boolean).join(" • ")
                : "";
    ...
    if (selected.includes("Address")) {
        const a=details.address||{};
        add("Address Size (UK) / Measurements", a.sizeMeasurements);
        add("Address Colour", a.colour);
        add("Address Quantity", a.quantity);
        add("Address Request", a.request);
    }
...
async function loadInvoicePricing() {
    ...
    if (trainingList) trainingList.innerHTML = renderRows(trainingInvoices, "No training invoice prices have been added yet.", "data-edit-training-invoice", "training").replace("<th>Products and Services</th>","<th>Training / Programme / Class</th>");
...
function setupTrainingInvoicePricingForm() {
    ...
    form.addEventListener("submit", async event => {
        ...
        try {
            ...
            await loadInvoicePricing();
            await loadTraining();
            returnToAdminList("[data-edit-training-invoice]", id || null);
        } catch (error) {
...
function setupInvoiceForm() {
    ...
    form.addEventListener("submit", async event => {
        ...
        try {
            ...
            await loadInvoicePricing();
            await loadProducts();
            returnToAdminList("[data-edit-invoice]", id || null);
        } catch (error) {
            console.error(error);
...
function setupAdminAutomaticCapitalisation() {
    if (document.documentElement.dataset.aprilsAdminCapsBound === "1") return;
    document.documentElement.dataset.aprilsAdminCapsBound = "1";
    const skip = new Set(["email","url","password","tel","number","date","time","hidden"]);
    const shouldSkip = field => skip.has(String(field?.type||"").toLowerCase()) || /email|url|password|phone|whatsapp|website|link|key|slug/i.test(String(field?.name||"")+" "+String(field?.id||""));
    const apply = field => {
        if (!field || shouldSkip(field)) return;
        const value=String(field.value||"");
        if (field.tagName === "TEXTAREA") field.value=value.replace(/(^|[.!?]\s+)([a-z])/g,(_,prefix,letter)=>prefix+letter.toUpperCase());
        else field.value=value.replace(/(^|\s+)([a-z])/g,(_,prefix,letter)=>prefix+letter.toUpperCase());
    };
