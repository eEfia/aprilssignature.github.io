...
(function(){
 ...
 async function save(key,val){if(window.safeSettingUpsert)return window.safeSettingUpsert(key,JSON.stringify(val));const d=dbif(!d)throw new Error("Supabase is unavailable");const old=await d.from("settings").select("id").eq("setting_key",key).limit(1);if(old.error)throw old.error;if(old.data?.length)return d.from("settings").update({setting_value:JSON.stringify(val),updated_at:new Date().toISOString()}).eq("id",old.data[0].id);return d.from("settings").insert({setting_key:key,setting_value:JSON.stringify(val),updated_at:new Date().toISOString()})}
