...
(function(){
 ...
 async function getInventory(){return(await rows()).filter(r=>String(r.setting_key||"").startsW("inventory_item_")).map(r=>{try{return{...JSON.parse(r.setting_value||"{}"),id:r.id,setting_key:r.setting_key}}catch(_){return null}}).filter(x=>x&&x.name).sort((a,b)=>Number(a.display_order||9999)-Number(b.display_order||9999))}
